...
function ensureStore() {
  ...
  store.workspaces = store.workspaces.map((workspace) => {
    ...
    const normalizedBookings = Array.isArray(workspace.bookings)
      ? workspace.bookings.map((booking) => normalizeWorkspaceBookingRecord(booking))
      : [];
    const normalizedScheduled = Array.isArray(workspace.scheduledMessages)
      ? workspace.scheduledMessages.map((s) => ({
          ...
          status: sanitizeChoice(s.status || "pending", ["pending", "sent", "failed", "cancelled"], "pending"),
    ...
    if (!Array.isArray(workspace.scheduledMessages)) {
      changed = true;
    }
    if (!Array.isArray(workspace.media)) {
      changed = true;
    }
...
function bookingIntentFromText(text) {
  ...
  return /(book|booking|schedule|slot|appointment|call|meeting|demo)/i.test(input);
}
...
function initBookingRecord(input) {
  const reminderMinutes = parseReminderList(input.reminderMinutes).length > 0
  ...
  return {
    ...
    reminders: reminderMinutes.map((minutesBefore) => ({
      minutesBefore,
      sentAt: "",
    })),
    noShowSentAt: "",
...
function normalizeWorkspaceBookingRecord(raw) {
  const normalized = initBookingRecord({
    ...
    reminderMinutes: (Array.isArray(raw?.reminders) ? raw.reminders.map((r) => r.minutesBefore) : []).join(","),
  });
...
function ensureGoogleCalendarClient() {
  if (!bookingApiReady()) {
    throw new Error("Booking API is not configured. Set BOOKING_ENABLED and Google Calendar credentials in .env.");
  }
...
async function findAvailableSlots({ timezone, limit = 8 }) {
  ...
  for (let dayOffset = 0; dayOffset <= BOOKING_LOOKAHEAD_DAYS; dayOffset += 1) {
    ...
    for (let cursor = new Date(start); cursor < end; cursor = new Date(cursor.getTime() + slotMs)) {
      ...
      if (slotEnd > end || slotStart <= new Date(now.getTime() + bufferMs)) {
        continue;
      }
...
function resolveMediaPath(workspace, mediaId) {
  ...
  const absPath = path.isAbsolute(mediaRec.path) ? mediaRec.path : path.join(DATA_DIR, mediaRec.path);
  ...
  try {
    ...
    for (let index = 0; index < recipients.length; index += 1) {
      ...
      for (const baseMsg of messages) {
        ...
        try {
          // If mediaId override provided, attempt to send media with optional caption
          if (overrides.mediaId) {
            ...
            await runtime.client.sendMessage(chatId, media, { caption: outgoingMessage || undefined });
          } else {
...
async function sendBookingIntentOptions(workspace, runtime, leadId, leadName, timezone) {
  if (!runtime.client || !runtime.ready) {
    return { sent: false, reason: "client_not_ready", slots: [] };
  }
  const slots = await findAvailableSlots({ timezone, limit: 3 });
  if (slots.length === 0) {
    const waitMsg = "I couldn't find open call slots right now. Please share your preferred time window and I will confirm manually.";
  ...
  const lines = slots.map((slot, idx) => `${idx + 1}) ${formatSlotForHumans(slot.startAt, slot.timezone)}`);
...
async function processWorkspaceBookingReminders(workspace) {
  ...
  ensureWorkspaceBookings(workspace);
  if (workspace.bookings.length === 0) {
    return false;
  }
  ...
  for (const booking of workspace.bookings) {
    ...
    const leadFirstName = sanitizeText(booking.leadName, bookingLeadName(leadId, workspace, "there")).split(" ")[0];

    if (!Array.isArray(booking.reminders)) {
      booking.reminders = BOOKING_REMINDER_MINUTES.map((minutesBefore) => ({ minutesBefore, sentAt: "" }));
      changed = true;
    }
    ...
    if (
      ...
      now.getTime() > (start.getTime() + ((BOOKING_NO_SHOW_GRACE_MINUTES + BOOKING_SLOT_MINUTES) * 60 * 1000))
    ) {
      try {
        const message = "Looks like we missed the call. Want me to share the next available slots so we can rebook quickly?";
        await runtime.client.sendMessage(leadId, message);
...
async function processWorkspaceScheduledMessages(workspace) {
  if (!Array.isArray(workspace.scheduledMessages) || workspace.scheduledMessages.length === 0) {
    return false;
  }
  const runtime = getRuntime(workspace.id);
    ...
    try {
      ...
      if (sched.mediaId) {
        ...
        for (const chatId of recipients) {
          const media = MessageMedia.fromFilePath(resolved.absPath);
          await runtime.client.sendMessage(chatId, media, { caption: messageText || undefined });
          appendReport(workspace, {
            kind: "outgoing",
            source: "scheduled",
            ok: true,
...
app.get("/api/workspaces/:workspaceId/media", requireAuth, (req, res) => {
...
app.post("/api/workspaces/:workspaceId/schedules", requireAuth, (req, res) => {
  const workspace = requireWorkspace(req, res, "admin");
  if (!workspace) return;
  try {
    ...
    workspace.scheduledMessages = Array.isArray(workspace.scheduledMessages) ? workspace.scheduledMessages : [];
    const rec = {
      id,
      message,
...
app.get("/api/workspaces/:workspaceId/schedules", requireAuth, (req, res) => {
  const workspace = requireWorkspace(req, res, "member");
  if (!workspace) return;
  res.json({ ok: true, scheduled: Array.isArray(workspace.scheduledMessages) ? workspace.scheduledMessages : [] });
});

app.delete("/api/workspaces/:workspaceId/schedules/:scheduleId", requireAuth, (req, res) => {
  const workspace = requireWorkspace(req, res, "admin");
  if (!workspace) return;
  ...
  if (idx === -1) {
    return res.status(404).json({ ok: false, error: "Scheduled message not found" });
  }
...
app.get("/api/workspaces/:workspaceId/bookings", requireAuth, async (req, res) => {
  try {
    const workspace = getWorkspace(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ ok: false, error: "Workspace not found" });
    if (!hasWorkspaceRole(workspace, req.user.id, "member")) {
...
app.post("/api/workspaces/:workspaceId/booking/intent", requireAuth, async (req, res) => {
  try {
    const workspace = getWorkspace(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ ok: false, error: "Workspace not found" });
    ...
    if (!intent) {
      return res.json({ ok: true, intent: false, slots: [] });
    }
...
app.get("/api/workspaces/:workspaceId/booking/slots", requireAuth, async (req, res) => {
  try {
    const workspace = getWorkspace(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ ok: false, error: "Workspace not found" });
    if (!hasWorkspaceRole(workspace, req.user.id, "admin")) {
...
app.post("/api/workspaces/:workspaceId/booking/confirm", requireAuth, async (req, res) => {
  try {
    const workspace = getWorkspace(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ ok: false, error: "Workspace not found" });
    if (!hasWorkspaceRole(workspace, req.user.id, "admin")) {
    ...
    const leadName = sanitizeText(req.body?.leadName, bookingLeadName(leadId, workspace, "Lead"));
    const timezone = bookingTimezone(req.body?.timezone || BOOKING_TIMEZONE);
    const startAt = new Date(req.body?.startAt || "");
    if (Number.isNaN(startAt.getTime())) {
      return res.status(400).json({ ok: false, error: "startAt must be a valid ISO datetime" });
    }
    const durationMinutes = Math.max(15, Number.parseInt(String(req.body?.durationMinutes || BOOKING_SLOT_MINUTES), 10) || BOOKING_SLOT_MINUTES);
    ...
    const booking = initBookingRecord({
      ...
      meetingLink: sanitizeText(event.hangoutLink, ""),
      notes,
      reminderMinutes: sanitizeText(req.body?.reminderMinutes, BOOKING_REMINDER_MINUTES.join(",")),
    });
    workspace.bookings.push(booking);
    updateLeadStatus(workspace, {
      from: leadId,
    ...
    if (runtime.client && runtime.ready && req.body?.notifyLead !== false) {
      const when = formatSlotForHumans(booking.startAt, booking.timezone);
      const meetLine = booking.meetingLink ? `\nMeeting link: ${booking.meetingLink}` : "";
      await runtime.client.sendMessage(leadId, `Your call is confirmed for ${when}.${meetLine}`.trim());
    }
...
app.post("/api/workspaces/:workspaceId/booking/cancel", requireAuth, async (req, res) => {
  try {
    const workspace = getWorkspace(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ ok: false, error: "Workspace not found" });
    if (!hasWorkspaceRole(workspace, req.user.id, "admin")) {
    ...
    if (booking.calendarEventId) {
      await cancelCalendarBookingEvent(booking.calendarEventId);
    }
    ...
    const runtime = getRuntime(workspace.id);
    if (runtime.client && runtime.ready && booking.leadId && req.body?.notifyLead !== false) {
      await runtime.client.sendMessage(
        booking.leadId,
...
app.post("/api/workspaces/:workspaceId/booking/reminder-sweep", requireAuth, async (req, res) => {
  try {
    const workspace = getWorkspace(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ ok: false, error: "Workspace not found" });
    if (!hasWorkspaceRole(workspace, req.user.id, "admin")) {
...
app.post("/api/workspaces/:workspaceId/send-custom", async (req, res) => {
  ...
  try {
    ...
    // If no text messages but mediaId provided, allow media-only campaign
    if (messages.length === 0 && !mediaId) {
      res.status(400).json({ ok: false, error: "At least one message or mediaId is required." });
      return;
    }

    // If a future sendAt is provided, create a scheduled message instead of sending now
    ...
    const results = await sendBulkMessage(workspace, runtime, messages.length > 0 ? messages : [""], {
      source: "custom",
      mode: req.body?.mode,
