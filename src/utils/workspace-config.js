...
function sanitizeWorkspaceConfig(input) {
  const clean = {
    ...
    AI_PROVIDER: sanitizeChoice(
      sanitizeText(input.AI_PROVIDER, "google"),
    ...
    OLLAMA_BASE_URL: sanitizeText(input.OLLAMA_BASE_URL, DEFAULT_CONFIG.OLLAMA_BASE_URL),
    ...
    // ─── Autonomous features ─────────────────────────────────────────────
    NURTURE_DRIP_ENABLED: input.NURTURE_DRIP_ENABLED === "true" ? "true" : "false",
    NURTURE_DRIP_STEPS: sanitizeMultilineText(input.NURTURE_DRIP_STEPS, DEFAULT_CONFIG.NURTURE_DRIP_STEPS),
    AUTO_REENGAGE_ENABLED: input.AUTO_REENGAGE_ENABLED === "true" ? "true" : "false",
    AUTO_REENGAGE_STALE_DAYS: sanitizeIntegerString(input.AUTO_REENGAGE_STALE_DAYS, DEFAULT_CONFIG.AUTO_REENGAGE_STALE_DAYS, 1, 90),
    AUTO_REENGAGE_MAX_ATTEMPTS: sanitizeIntegerString(input.AUTO_REENGAGE_MAX_ATTEMPTS, DEFAULT_CONFIG.AUTO_REENGAGE_MAX_ATTEMPTS, 1, 10),
    AUTO_ESCALATION_ENABLED: input.AUTO_ESCALATION_ENABLED === "true" ? "true" : "false",
    AUTO_ESCALATION_OPERATOR: sanitizeText(input.AUTO_ESCALATION_OPERATOR, DEFAULT_CONFIG.AUTO_ESCALATION_OPERATOR),
    AUTO_LEAD_ROUTING_ENABLED: input.AUTO_LEAD_ROUTING_ENABLED === "true" ? "true" : "false",
    AB_TEST_ENABLED: input.AB_TEST_ENABLED === "true" ? "true" : "false",
    AB_TEST_MIN_SENDS: sanitizeIntegerString(input.AB_TEST_MIN_SENDS, DEFAULT_CONFIG.AB_TEST_MIN_SENDS, 5, 1000),
    AUTO_DAILY_DIGEST_ENABLED: input.AUTO_DAILY_DIGEST_ENABLED === "true" ? "true" : "false",
    AUTO_DAILY_DIGEST_OPERATOR: sanitizeText(input.AUTO_DAILY_DIGEST_OPERATOR, DEFAULT_CONFIG.AUTO_DAILY_DIGEST_OPERATOR),
    AUTO_DAILY_DIGEST_HOUR: sanitizeIntegerString(input.AUTO_DAILY_DIGEST_HOUR, DEFAULT_CONFIG.AUTO_DAILY_DIGEST_HOUR, 0, 23),
    AUTO_OBJECTION_ENABLED: input.AUTO_OBJECTION_ENABLED === "true" ? "true" : "false",
    AUTO_CLEANUP_ENABLED: input.AUTO_CLEANUP_ENABLED === "true" ? "true" : "false",
    AUTO_CLEANUP_STALE_DAYS: sanitizeIntegerString(input.AUTO_CLEANUP_STALE_DAYS, DEFAULT_CONFIG.AUTO_CLEANUP_STALE_DAYS, 7, 365),
    AUTO_TIMEZONE_ENABLED: input.AUTO_TIMEZONE_ENABLED === "true" ? "true" : "false",
    AUTO_TAGGING_ENABLED: input.AUTO_TAGGING_ENABLED === "true" ? "true" : "false",

    // ─── Agentic features (v2) ───────────────────────────────────────────
    OUTBOUND_PROSPECTING_ENABLED: input.OUTBOUND_PROSPECTING_ENABLED === "true" ? "true" : "false",
    OUTBOUND_MAX_DAILY: sanitizeIntegerString(input.OUTBOUND_MAX_DAILY, DEFAULT_CONFIG.OUTBOUND_MAX_DAILY, 1, 100),
    OUTBOUND_COOLDOWN_HOURS: sanitizeIntegerString(input.OUTBOUND_COOLDOWN_HOURS, DEFAULT_CONFIG.OUTBOUND_COOLDOWN_HOURS, 1, 168),
    OUTBOUND_TEMPLATE: sanitizeMultilineText(input.OUTBOUND_TEMPLATE, DEFAULT_CONFIG.OUTBOUND_TEMPLATE),
    GOAL_PLANNER_ENABLED: input.GOAL_PLANNER_ENABLED === "true" ? "true" : "false",
    GOAL_TYPE: sanitizeChoice(sanitizeText(input.GOAL_TYPE, DEFAULT_CONFIG.GOAL_TYPE), ["bookings", "hot_leads", "qualified", "replies", "revenue"], DEFAULT_CONFIG.GOAL_TYPE),
    GOAL_WEEKLY_TARGET: sanitizeIntegerString(input.GOAL_WEEKLY_TARGET, DEFAULT_CONFIG.GOAL_WEEKLY_TARGET, 1, 1000),
    PROMPT_TUNING_ENABLED: input.PROMPT_TUNING_ENABLED === "true" ? "true" : "false",
    OFFER_AUTHORITY_ENABLED: input.OFFER_AUTHORITY_ENABLED === "true" ? "true" : "false",
    OFFER_MAX_DISCOUNT_PCT: sanitizeIntegerString(input.OFFER_MAX_DISCOUNT_PCT, DEFAULT_CONFIG.OFFER_MAX_DISCOUNT_PCT, 1, 100),
    OFFER_MIN_LEAD_SCORE: sanitizeIntegerString(input.OFFER_MIN_LEAD_SCORE, DEFAULT_CONFIG.OFFER_MIN_LEAD_SCORE, 0, 100),
    OFFER_BASE_PRICE: sanitizeText(input.OFFER_BASE_PRICE, DEFAULT_CONFIG.OFFER_BASE_PRICE),
    OFFER_CURRENCY: sanitizeText(input.OFFER_CURRENCY, DEFAULT_CONFIG.OFFER_CURRENCY),
    OFFER_ALLOW_PAYMENT_PLAN: input.OFFER_ALLOW_PAYMENT_PLAN === "true" ? "true" : "false",
    OFFER_MAX_PER_LEAD: sanitizeIntegerString(input.OFFER_MAX_PER_LEAD, DEFAULT_CONFIG.OFFER_MAX_PER_LEAD, 1, 10),
    OFFER_URGENCY_WINDOW: sanitizeText(input.OFFER_URGENCY_WINDOW, DEFAULT_CONFIG.OFFER_URGENCY_WINDOW),
    SELF_HEALING_ENABLED: input.SELF_HEALING_ENABLED === "true" ? "true" : "false",

    // ─── WhatsApp Alerts & Auto-Reports ──────────────────────────────────
    WHATSAPP_ALERTS_ENABLED: input.WHATSAPP_ALERTS_ENABLED === "true" ? "true" : "false",
    WHATSAPP_ALERTS_OPERATOR: sanitizeText(input.WHATSAPP_ALERTS_OPERATOR, DEFAULT_CONFIG.WHATSAPP_ALERTS_OPERATOR),
    WHATSAPP_ALERTS_EVENTS: sanitizeText(input.WHATSAPP_ALERTS_EVENTS, DEFAULT_CONFIG.WHATSAPP_ALERTS_EVENTS),
    WHATSAPP_ALERTS_REPORT_INTERVAL_HRS: sanitizeIntegerString(input.WHATSAPP_ALERTS_REPORT_INTERVAL_HRS, DEFAULT_CONFIG.WHATSAPP_ALERTS_REPORT_INTERVAL_HRS, 1, 24),
